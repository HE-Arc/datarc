...
def file(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({'status' : 'error body'})
        if 'name' and 'public' and 'token' in data.keys():
            if valid_file_name(data['name']) and valid_file_public(data['public']) and valid_user_token(data['token']):
        ...
        if 'p' in request.GET.keys():
            if valid_file_url(request.GET['p']):
                files =  File.objects.all().filter(url=request.GET['p'])
                if len(files) != 1:
                    return JsonResponse({'status' : 'error file doesnt exist'})
                my_file = files[0]
                
                if my_file.public == True:
                    return  JsonResponse({'status' : 'ok public'})
                else:
                    try:
                        data = json.loads(request.body) 
                    except:
                    ...
                    if 'token' in data.keys():
                        if valid_user_token(data['token']):
                            user = getUser(data['token'])
                            if user == -1:
                                return JsonResponse({'status' : 'error user doesnt exist'})
                            files = UserAccess.objects.all().filter(userId=user.id, fileId=my_file.id)
                            if len(files) != 1:
                                return JsonResponse({'status' : 'error user doesnt have access'})
                            return JsonResponse({'status' : 'ok private'})
                        else:
                            return JsonResponse({'status' : 'error wrong user token'})  
